...
#  print "configuration=".$form{"configuration"}."\n";
  ...
  print startform;
    ...
    # 4 GHz continuum?
    print "<tr>";
    print "<th align=\"right\">4 GHz continuum?:</th>";
    if ($form{"parameters_present"}==0){
	$form{"4ghz"}="no";
    }
    print "<td align=\"left\"><input type=\"checkbox\" name=\"4ghz\" value=\"yes\"";
    if ($form{"4ghz"} eq "yes"){
	print " checked=\"checked\"";
    }
    print " />";
    ...
    # a user-specified zoom frequency
    print "<tr>";
    print "<th align=\"right\">Specific Zoom Freq:</th>";
    if ($form{"parameters_present"}==0){
	$form{"s_zoom"}=0;
    }
    print "<td align=\"left\"><input type=\"text\" name=\"s_zoom\" value=\"".
	$form{"s_zoom"}."\" size=\"6\" maxlength=\"6\" ".
	"title=\"A specific zoom frequency you want information about (will be shown".
	" as 'S_Zoom')\" />";
    print "</td></tr>\n";
    
    # how many new 4cm receivers to assume
    print "<tr>";
    print "<th align=\"right\">Number of 4cm receivers:</th>";
    if ($form{"parameters_present"}==0) {
	$form{"n4cm"}=3;
    }
    print "<td align=\"left\"><input type=\"text\" name=\"n4cm\" value=\"".
	$form{"n4cm"}."\" size=\"3\" maxlength=\"1\" ".
	"title=\"The number of upgraded 4cm to include in the calculation\" />";
    print "</td></tr>\n";
...
sub print_usage_information {
    ...
    print h2("NEW: User-friendly calculator!");
    print "Navigate <a href=\"http://www.narrabri.atnf.csiro.au/myatca/sensitivity_calculator.html\">here</a>";
    print " to try it.";
