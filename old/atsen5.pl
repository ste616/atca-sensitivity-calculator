...
#	$halim = 6;
	...
	$twelvemm = 1;
    }elsif ($form{"frequency"}>=8000 && $form{"frequency"}<=9152) {
    ...
    }elsif ($form{"frequency"}>=2240 && $form{"frequency"}<=2624) {
	$tsys1 = 2.5 * interp(int(@sf),@st,@sf,$form{"frequency"});
    }elsif ($form{"frequency"}>=1216 && $form{"frequency"}<=1751) {
	$tsys1 = 2 * interp(int(@lf),@lt,@lf,$form{"frequency"});
    }else {
	print "<b>The frequency you have selected is out of the valid range, please try again.<br />Valid ranges are: 1216 to 1751 MHz; 2240 to 2624 MHz; 4288 to 6720 MHz; 8000 to 9152 MHz; 16000 to 25000 MHz; 30000 to 50000 MHz; and 83870 to 104780 MHz.</b><br />";
	return;
    } 
