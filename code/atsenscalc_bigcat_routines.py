...
#continuumBandwidth = 1920.0 # MHz bandwidth of the continuum bands.
...
def calculateSensitivity(rmsTemplate, nAnts, args):
    ...
    if (rmsN > 0):
        ...
        rmsZoom = rmsSpectral * math.sqrt(float(args.zoom_channels))
    else:
        rmsSpectral = None
        totalBandwidth = None
        rmsContinuum = None
...
def channelResolution(args):
    # Return the width of each type of channel for a given correlator specification.
    return { 'continuum': (128.0 / args.subband_channels),
             'zoom': (args.zoom_bandwidth / args.zoom_channels) }
