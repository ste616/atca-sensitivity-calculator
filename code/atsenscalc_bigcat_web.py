...
if __name__ == "__main__":
    ...
    # The number of channels per subband for the spectral output.
    if ("subband_channels" in form):
        fargs['subband_channels'] = int(form['subband_channels'].value)
    else:
        fargs['subband_channels'] = 128

    # The number of subbands that make up the window.
    if ("number_subbands" in form):
        fargs['number_subbands'] = int(form['number_subbands'].value)
    else:
        fargs['number_subbands'] = 15
    ...
    # Default 1.
    if ("zoom_bandwidth" in form):
        fargs['zoom_bandwidth'] = float(form['zoom_bandwidth'].value)
    else:
        fargs['zoom_bandwidth'] = 1.0

    # The number of channels in a zoom, before smoothing.
    if ("zoom_channels" in form):
        fargs['zoom_channels'] = int(form['zoom_channels'].value)
    else:
        fargs['zoom_channels'] = 2048
